...
CREATE TABLE IF NOT EXISTS hero_slides (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  slide_number INTEGER NOT NULL CHECK (slide_number IN (1,2,3)),
  title TEXT NOT NULL,
  subtitle TEXT,
  message TEXT,
...
CREATE TABLE IF NOT EXISTS word_for_today (
  ...
  reference VARCHAR(100) NOT NULL,  -- e.g. "Colossians 3:16"
  reflection TEXT,
  is_active BOOLEAN DEFAULT true,
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW()
...
CREATE TABLE IF NOT EXISTS resources (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  category VARCHAR(255) NOT NULL,         -- e.g. "Anchor Devotional"
  description TEXT,
  drive_url TEXT NOT NULL,                -- Google Drive folder/file URL
  icon VARCHAR(50),                       -- emoji
  is_devotional BOOLEAN DEFAULT false,    -- Special display for Anchor Devotional
  display_order INTEGER DEFAULT 0,
  is_active BOOLEAN DEFAULT true,
  updated_at TIMESTAMPTZ DEFAULT NOW()
);
...
-- Allow public read on public tables
...
-- Allow public inserts for submissions/subscriptions
